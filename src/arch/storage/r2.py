...
class R2Storage:
    ...
    def from_config(cls, config_path: str | Path | None = None) -> "R2Storage":
        ...
        Args:
            config_path: Path to credentials JSON. Defaults to ~/.config/cosilico/r2-credentials.json

        Returns:
            Configured R2Storage instance
        """
        if config_path is None:
            config_path = Path.home() / ".config" / "cosilico" / "r2-credentials.json"
    ...
    def upload_state_statute(
        ...
        metadata = {
            ...
            'fetched-at': (fetched_at or datetime.now(timezone.utc)).isoformat(),
        }
