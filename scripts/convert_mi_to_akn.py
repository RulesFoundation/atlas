...
def parse_mcl_chapter(filepath: Path) -> dict:
    ...
    text = re.sub(r'<\?xml[^>]*encoding=["\']utf-16["\'][^>]*\?>', '<?xml version="1.0" encoding="UTF-8"?>', text, flags=re.IGNORECASE)
...
def main():
    ...
    for filepath in sorted(xml_files):
        try:
            ...
            print(f"  -> {output_filename} ({len(data['divisions'])} divisions, {section_count} sections)")
