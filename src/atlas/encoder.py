...
SYSTEM_PROMPT = """You are an expert at encoding US tax and benefit law into Cosilico DSL.
...
def encode_section(section: Section, model: str = "claude-sonnet-4-20250514") -> Encoding:
    ...
    user_prompt = f"""Encode the following statute section into Cosilico DSL:
...
"""
    ...
    dsl = _extract_code_block(content, "cosilico")
...
def encode_and_save(
    ...
    Args:
        ...
        output_dir: Directory to save output (e.g., ~/.cosilico/workspace)
        model: Claude model to use
    ...
    # Save DSL
    (section_dir / "rules.cosilico").write_text(encoding.dsl)
